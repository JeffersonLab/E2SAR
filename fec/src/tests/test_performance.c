...
#define TEST_ITERATIONS 100000000  // Number of iterations for performance tests (100 million)
...
void print_decoder_header(int num_erasures) {
    ...
    printf("========================================================================\n");
    printf("Configuration:\n");
    printf("  Iterations: %d (encoder iterations / 10)\n", TEST_ITERATIONS / 10);
    printf("  Batch size: %d\n", BATCH_SIZE);
    printf("  Block size: %d\n", BLOCK_SIZE);
    printf("  RS Code: (10,8) - 8 data symbols, 2 parity symbols\n");
...
void test_decoder_performance_with_erasures(rs_model *rs, rs_decode_table *decode_table,
    ...
    // Decoder tests use 1/10th the iterations of encoder tests
    long long decoder_iterations = TEST_ITERATIONS / 10;
    double time_taken, data_rate, baseline_time = 0;
    int test_num = 0;
    int correct;
    ...
    double start_usec = get_time_usec();
    for (long long i = 0; i < decoder_iterations; i++) {
        rs_decode_erasures(rs, &codeword, erasure_positions, num_erasures, &decoded);
    }
    double end_usec = get_time_usec();
    time_taken = (end_usec - start_usec) / 1e6;  // Convert microseconds to seconds
    data_rate = 1/1E6 * 4 * 8 * decoder_iterations / time_taken;  // 4 bits per nibble
    baseline_time = time_taken;
    ...
    printf("   Time: %.6f seconds\n", time_taken);
    printf("   Throughput: %.1f Mbps\n", data_rate);
    printf("   Operations/sec: %.0f\n", (double)decoder_iterations / time_taken);
    printf("   Speedup: 1.00x (baseline)\n");
    ...
    int num_batches = (decoder_iterations + BATCH_SIZE - 1) / BATCH_SIZE;
    ...
    printf("   Time: %.6f seconds\n", time_taken);
    printf("   Throughput: %.1f Mbps\n", data_rate);
    printf("   Operations/sec: %.0f\n", (double)num_batches * BATCH_SIZE / time_taken);
    printf("   Speedup: %.2fx\n", baseline_time / time_taken);
    printf("   Result: %s\n\n", correct ? "CORRECT" : "INCORRECT");
