...
ource /Users/baldin/workspaces/workspace-ejfat/E2SAR/.venv/bin/activate
PI Details can always be found in the [wiki](https://github.com/JeffersonLab/E2SAR/wiki) and in the [Doxygen site](https://jeffersonlab.github.io/E2SAR-doc/annotated.html). 

## v0.4.0
- PLEASE NOTE SEVERAL BREAKING CHANGES!
...
- BREAKING: Deprecated ReassemblerFlags.withLBHeader - the field has been removed, instead the logic uses the useCP flag to determine whether to expect (useCP==false) or not to expect (useCP==true) the LB header in the received packets. May break some test setups.
- BREAKING: Added checks in Segmenter and Reassembler to make sure the requested send or receive buffer size is actually honored (error is raised if the OS didn't accept it). Previous behavior was to silently succeed even if the OS allocated a smaller buffer. Note that if before you used the default 3M send/receive socket buffer sizing, but you have a default Linux setting of 128k, your code will exit with error. You will need to set sFlags.sndSocketBufSize and rFlags.rcvSocketBufSize to 65536 or similarly low value (or update your INI files)
...
- BREAKING: Updated for the new EJFAT_URI format that supports port ranges in data plane and allows IPv6 sync addresses. Now for local back-to-back testing it is now REQUIRED to specify a single (or multiple, depending on how many receive threads are running on the receiver side) port for the data address e.g. ejfats://token@127.0.0.1:18347/lb/1?data=127.0.0.1:19522&sync=127.0.0.1 . Using default ejfats://token@127.0.0.1:18347/lb/1?data=127.0.0.1&sync=127.0.0.1  will attempt to use the full default range of data ports 16364-32767 which will result in losses on the receive side. This should not affect anything using a production load balancer. "--multiport" INI option and "multiport" Segmenter flag have been deprecated. THIS IS POSSIBLY A BREAKING CHANGE FOR SOME TESTING SETUPS!
- Added a higher level Python interface on top of e2sar_py (`import e2sar`) shipping with Conda (courtesy of David Rogers [ORNL]). Incorporates https://github.com/frobnitzem/pye2sar natively into E2SAR. Note module name change - not pye2sar, but simply e2sar. 
