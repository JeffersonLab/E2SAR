...
# install Rust toolchain (rustup) to build/run udplbd2 natively (no docker).
# protoc (protobuf-compiler) and the C toolchain are installed above; the
# distro rustc is usually too old for udplbd2, so use rustup stable. rustup -y
# writes '. "$HOME/.cargo/env"' into the shell profiles, so later sessions get
# cargo on PATH.
if ! command -v cargo >/dev/null 2>&1; then
    curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y
fi

# install sqlx-cli (sqlite only, rustls to avoid OpenSSL) so udplbd2's
# compile-time-checked sqlx queries can be validated against a build-time SQLite
# schema (the checked-in .sqlx offline cache is unreliable). Needs cargo from the
# rustup step above.
