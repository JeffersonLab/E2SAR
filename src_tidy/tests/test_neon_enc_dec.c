...
#include "../neon/ejfat_rs_neon_decoder.h"
...
int init_comprehensive_decode_table(rs_decode_table *table) {
    printf("Initializing comprehensive decode table...\n");
    ...
    printf("Comprehensive decode table initialized with %d patterns\n", table->size);
    return 0;
}

void free_comprehensive_decode_table(rs_decode_table *table) {
    if (table->entries) {
        free(table->entries);
        table->entries = NULL;
    }
...
void test_encode_decode_cycle() {
    printf("\n=============== Testing Complete Encode/Decode Cycle ===============\n");
    
    // Initialize encoder and decoder
    rs_model *encoder = init_neon_rs_encoder();
    if (!encoder) {
        printf("Failed to initialize NEON encoder\n");
        return;
    }
    
    rs_decode_table decode_table;
    if (init_comprehensive_decode_table(&decode_table) != 0) {
        printf("Failed to initialize decode table\n");
        free_neon_rs_encoder(encoder);
        return;
    }
    ...
    for (int p = 0; p < num_patterns; p++) {
        ...
        // Encode
        neon_rs_encode(encoder, &original_data, &parity);
        printf("Parity: ");
        ...
        total_tests++;
        if (neon_rs_decode_table_lookup_v2(&decode_table, &codeword, erasures, 0, &decoded) == 0) {
            int correct = 1;
            for (int i = 0; i < 8; i++) {
        ...
        for (int err_pos = 0; err_pos < 8; err_pos++) {
            ...
            char original_symbol = corrupted.val[err_pos];
            ...
            total_tests++;
            if (neon_rs_decode_table_lookup_v2(&decode_table, &corrupted, erasures_err, 1, &decoded_err) == 0) {
                int correct = 1;
                for (int i = 0; i < 8; i++) {
    ...
    printf("\n=== Complete Encode/Decode Test Summary ===\n");
    ...
    // Clean up
    free_comprehensive_decode_table(&decode_table);
    free_neon_rs_encoder(encoder);
    printf("\n=============== Encode/Decode Cycle Tests Complete ===============\n");
}

// Comprehensive error correction test with systematic error injection
void test_systematic_error_correction() {
    printf("\n=============== Systematic Error Correction Test ===============\n");
    ...
    // Encode test message
    neon_rs_encode(encoder, &test_data, &parity);
    ...
    for (int err1 = 0; err1 < 7; err1++) {
        for (int err2 = err1 + 1; err2 < 8; err2++) {
            ...
            int result = neon_rs_decode_table_lookup_v2(&decode_table, &corrupted, erasures, 2, &decoded);
            if (result != 0) {
    ...
    printf("\n=== Systematic Error Correction Summary ===\n");
    ...
    free_comprehensive_decode_table(&decode_table);
    free_neon_rs_encoder(encoder);
    printf("\n=============== Systematic Error Correction Tests Complete ===============\n");
}
...
void test_pipeline_performance() {
    ...
    for (int i = 0; i < test_iterations; i++) {
        ...
        // Decode (no errors)
        int erasures[] = {};
        neon_rs_decode_table_lookup_v2(&decode_table, &codeword, erasures, 0, &decoded);
    }
    ...
    for (int i = 0; i < test_iterations; i++) {
        ...
        int erasures[] = { 3 };
        neon_rs_decode_table_lookup_v2(&decode_table, &codeword, erasures, 1, &decoded);
    }
    end_time = clock();
    ...
    free_comprehensive_decode_table(&decode_table);
    free_neon_rs_encoder(encoder);
    printf("=============== Pipeline Performance Tests Complete ===============\n");
}

int main() {
    printf("NEON Reed-Solomon Encoder/Decoder Integration Test\n");
    printf("==================================================\n");
    ...
    test_systematic_error_correction();
